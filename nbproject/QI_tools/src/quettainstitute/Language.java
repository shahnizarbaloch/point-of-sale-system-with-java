...
public class Language {
   ...
   public Database dbl; 
    ...
    public void load(String lang,int startSentence){
        ...
        languageName=lang; 
        Object[][] td=dbl.getTable("SELECT id,"+lang+" FROM LABELS WHERE ID>="+startSentence+" AND ID<"+(startSentence+100)+" ORDER BY ID ASC"); 
        int l=td.length; 
        for (int i = 0; i < l; i++) {
        sentence.setProperty(""+td[i][0], td[i][1]+"");     
        }
        Object[][] td2=dbl.getTable("SELECT FONT,KEYBOARD,TEXT_ALIGN FROM LANGUAGE WHERE ID="+languageID); 
    ...
    public Font getFont(int fontNo){
       ...
       try{
     return new Font(fontFace,0,(int)dbl.get("SELECT FONT_SIZE"+fontNo+" FROM LANGUAGE WHERE ID="+languageID)); 
       }catch(Exception e){
       e.printStackTrace();
       }
       return f; 
...
public int getLanguageID(String language){
    int tr=-1; 
    try{
    tr=(int)dbl.get("SELECT ID FROM LANGUAGE WHERE LANGUAGE = '"+language+"'"); 
    }catch(Exception e){}
return tr; 
}
...
String fromLanguage=""; 
    ...
    try {
        ...
        switchKeyboard(keyBoard,round+1); 
...
public String getSentence(int sentenceID){
return dbl.get("SELECT "+languageName+" FROM SENTENCES WHERE ID="+sentenceID).toString(); 
